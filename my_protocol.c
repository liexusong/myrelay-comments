...
int parse_init(buf_t *buf, my_auth_init_t *init)
{
    ...
    init->pktlen = G3(&ptr);    // package length
    init->pktno = G1(&ptr);     // package number
    init->prot_ver = G1(&ptr);  // protocol version
...
int parse_com(buf_t *buf, cli_com_t *com)
{
    ...
    com->pktlen = G3(&ptr);  // package length
    com->pktno = G1(&ptr);   // package number
    com->comno = G1(&ptr);   // command op

    len = com->pktlen - 1;   // command length
