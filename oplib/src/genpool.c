...
typedef struct{
    void *chunk_addr;                 // for free()
    struct list_head link;            // link all chunks
    struct list_head used_pages_head; // used pages list
    struct list_head free_pages_head; // free pages list
} chunk_t;

typedef struct{
    chunk_t *belong_chunk;  // which chunk belong to
    struct list_head link;  // link all pages
...
static int _alloc_a_chunk(genpool_handler_t *g)
{
    ...
    n = g->pages_per_chunk; // 预分配的chunk数
    ...
    INIT_LIST_HEAD(&(c->used_pages_head)); // 初始化chunk的used_pages_list
    INIT_LIST_HEAD(&(c->free_pages_head)); // 初始化chunk的free_pages_list
...
inline void *genpool_alloc_page(genpool_handler_t *g)
{
    ...
    if(list_empty(&(g->used_chunks_head))){
        if(list_empty(&(g->free_chunks_head))){
            ...
            c = list_first_entry(&(g->free_chunks_head), chunk_t, link); // 从free_pages_list中获取到一个page
            list_del_init(&(c->link));
            list_add_tail(&(c->link), &(g->used_chunks_head)); // 添加到used_chunks_head中
            g->free_chunks--;
        }
    }
